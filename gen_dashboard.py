...
<h2>All nine real bugs, caught by testing before they shipped</h2>
...
<div class="bug">
  <div class="title">9. A residual-add mistaken for a bias-add &mdash; the mirror image of bug #4, found independently in a different fusion pass</div>
  A new pass, <code>fuse_add_layernorm</code>, fused <code>Add(x, bias)</code> into <code>AddLayerNorm</code> whenever an Add fed a LayerNorm, without checking whether the second operand was a genuine full-tensor residual or an ordinary (cols,) bias Param. The kernel assumed both operands were full (rows, cols) tensors; handed a (64,) bias for a (29, 64) input, it read past the small array for every row past the first — silent NaN, not a crash. Found by the extended fuzzer, which also revealed the fuzzer had zero actual coverage of this fusion path until a dedicated grammar rule was added.
  <div class="fix">Fixed the same way as bug #4: require the second operand to originate from something other than a Param before treating it as a broadcast-eligible residual. Verified clean across 1000 random graphs genuinely exercising all three fusion mechanisms (217 elementwise + 843 diamond + 851 AddLayerNorm fusions).</div>
</div>
